...
class Agent:
    def __init__(self, state_size, batch_size, is_eval=False,
                 ...
                 lr_actor=0.0001, lr_critic=0.001, optimizer_epsilon=K.epsilon()):
    ...
    def learn(self, experiences):
        states = np.vstack([e.state for e in experiences if e is not None]).astype(
            np.float32).reshape(-1, self.state_size)
        actions = np.vstack([e.action for e in experiences if e is not None]).astype(
